...
dev:
	RESTQL_PORT=9000 RESTQL_HEALTH_PORT=9001 RESTQL_DEBUG_PORT=9002 RESTQL_ENV=development CGO_ENABLED=0 go run -ldflags="-X main.build=$(vcs_ref)" -mod=vendor cmd/restQL/main.go

unit:
	cd internal && CGO_ENABLED=0 go test -mod=vendor -count=1 ./...
