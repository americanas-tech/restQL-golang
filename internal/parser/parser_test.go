...
func TestQueryParser(t *testing.T) {
	tests := []struct {
		...
		//},
		{
			...
			domain.Query{Statements: []domain.Statement{{Method: "from", Resource: "hero", With: domain.Params{Values: map[string]interface{}{"id": domain.Flatten{domain.Json{[]interface{}{1, 2}}}}}}}},
			"from hero with id = [1, 2] -> flatten -> json",
		},
		//{
		...
		//	domain.Query{Statements: []domain.Statement{{Method: "to", Resource: "hero", With: domain.Params{Body: domain.Flatten{Target: domain.Variable{Target: "hero"}}, Values: map[string]interface{}{"name": "batman"}}}}},
		//	`to hero with $hero -> flatten, name = "batman"`,
		//},
		//{
		//	"Unique from statement and only filters with match function",
		//	domain.Query{Statements: []domain.Statement{{Method: "from", Resource: "hero", Only: []interface{}{domain.Match{Target: []string{"name"}, Arg: regexp.MustCompile("^Super")}, []string{"weapons"}}}}},
